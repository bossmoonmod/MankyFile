...
$db = getDB();
$hStmt = $db->prepare("UPDATE tasks SET last_heartbeat = CURRENT_TIMESTAMP WHERE id = ?");
$hStmt->execute([$task_id]);

// Handle Abandoned logic: 120 seconds of no heartbeat = dead task
$db->exec("UPDATE tasks SET status = 'failed' WHERE status = 'processing' AND (julianday('now') - julianday(last_heartbeat)) * 86400 > 120");
...
} elseif ($task['type'] == 'pdf-to-word') {
    $file_ext = 'docx';
    $mime_type = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
} elseif ($task['type'] == 'pdf-to-image') {
    $file_ext = 'zip';
    $mime_type = 'application/zip';
} elseif ($task['type'] == 'image-resize') {
    $file_ext = 'jpg';
    $mime_type = 'image/jpeg';
} elseif ($task['type'] == 'image-convert') {
    $file_ext = 'webp';
...
if ($task['type'] == 'unlock') {
    $filename_on_disk = $task_id . '_processed.pdf'; // Match worker.py output
}

// Ensure OUTPUT_DIR has a single trailing slash
$out_dir = rtrim(OUTPUT_DIR, '/\\') . '/';
$file_path = $out_dir . $filename_on_disk;

// If not found, check standard API processed suffix or fallback name
if (!file_exists($file_path)) {
    $file_path = $out_dir . $task_id . '_processed.' . $file_ext;
}
if (!file_exists($file_path) && $task['type'] == 'unlock') {
    $file_path = $out_dir . $task_id . '_unlocked.pdf'; // Reverse legacy check
}
    ...
    foreach ($candidates as $ext => $mime) {
        ...
        foreach ($possible_paths as $check_path) {
            if (file_exists($check_path)) {
                ...
                $new_name = str_replace(['.pdf', '.pptx', '.xlsx', '.docx', '.zip', '.jpg', '.png', '.webp'], '.' . $ext, $download_name);
