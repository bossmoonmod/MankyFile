<?php
require_once 'db.php';

// 1. Authenticate
$headers = getallheaders();
$key = isset($headers['X-API-KEY']) ? $headers['X-API-KEY'] : '';
if ($key !== API_KEY) {
    http_response_code(403);
    echo json_encode(['error' => 'Invalid API Key']);
    exit;
}

// 2. Handle Upload
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
    $file = $_FILES['file'];
    $job_id = uniqid('task_');
    $ext = pathinfo($file['name'], PATHINFO_EXTENSION);
    $input_path = UPLOAD_DIR . $job_id . '.' . $ext;
    
    if (move_uploaded_file($file['tmp_name'], $input_path)) {
        // Save to DB
        $db = getDB();
        $stmt = $db->prepare("INSERT INTO tasks (id, status, original_filename) VALUES (?, 'pending', ?)");
        $stmt->execute([$job_id, $file['name']]);
        
        // 3. Trigger Python Worker in Background
        // Command: nohup python3 worker.py [input] [output] [job_id] > /dev/null 2>&1 &
        $output_path = OUTPUT_DIR . $job_id . '_unlocked.pdf';
        $script = __DIR__ . '/worker.py';
        
        // Escape args
        $cmd = sprintf(
            '%s %s %s %s %s > /dev/null 2>&1 &',
            PYTHON_PATH,
            escapeshellarg($script),
            escapeshellarg($input_path),
            escapeshellarg($output_path),
            escapeshellarg($job_id)
        );
        
        // Execute (Windows/Linux compatible fallback logic)
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            pclose(popen("start /B " . $cmd, "r"));
        } else {
            exec($cmd); 
        }

        echo json_encode(['status' => 'ok', 'task_id' => $job_id]);
    } else {
        http_response_code(500);
        echo json_encode(['error' => 'Failed to save file']);
    }
} else {
    echo json_encode(['error' => 'No file uploaded']);
}
?>
